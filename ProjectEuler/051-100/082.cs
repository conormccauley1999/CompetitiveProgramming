...
namespace _082
{
    ...
    class Program
    {
        ...
        public static void Main(string[] args)
        {
            ...
            int[][] M = LoadMatrix("../files/82.txt");
            int min = Constants.infinity;

            for (int y1 = 0; y1 < M.Length; y1++)
            {
                int s = Dijkstra(0, y1, M);
                if (s < min) min = s;
                Console.WriteLine("(0, {0}) -> (79, *) => {1} ; min = {2}", y1, s, min);
            }
        ...
        private static int Dijkstra(int sx, int sy, int[][] M)
        {
            ...
            while (Q.Count != 0)
            {
                ...
                foreach (Vertex v in n)
                {
                    ...
                    if (alt < v.dist)
                    {
                        v.dist = alt;
                        v.prev = u;
                        if (v.p.X == (M.Length - 1) && v.dist < min) min = v.dist;
                    }
